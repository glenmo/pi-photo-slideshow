...
A fullscreen photo slideshow served by Apache2 on a Raspberry Pi 5. Photos are displayed with a fade-to-black transition and filename-based captions. New photos added over the network appear automatically within 60 seconds — no restart required.
...
- Apache2 web server
- Python 3
- Chromium browser (included with Raspberry Pi OS)
...
Captions are generated automatically from filenames. For best results:

- Use underscores or hyphens as word separators
- No need for spaces (they cause issues over `scp`)
...
**Site not found at rubberduck.local**
...
~/
└── scan_photos.py      # run every minute via cron
```
